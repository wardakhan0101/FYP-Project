...
class _PronunciationReportScreenState extends State<PronunciationReportScreen>
  ...
  Widget build(BuildContext context) {
    ...
    // Accent UI is intentionally hidden from end users — the heuristic
    // detector still runs server-side and the dev test screen surfaces
    // it, but the per-session card is suppressed until the detector is
    // accurate enough to ship. The `detected_accent` / `accent_confidence`
    // / `accent_evidence` fields on `data` are still populated and
    // persisted; only the rendered card is removed.
    return Scaffold(
      backgroundColor: _bg,
